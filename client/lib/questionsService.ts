import { QuestionCard, SheetyResponse, CategoryType } from '@shared/sheety';

const SHEETY_API_URL = 'https://api.sheety.co/8066c7c9e5203e8d27825f3e41eebddb/openQuestionsDataBase/sheet1';
...
export async function fetchQuestionsByCategory(category: CategoryType): Promise<QuestionCard[]> {
  try {
    ...
    const filteredQuestions = data.sheet1.filter(
...
export async function fetchAllQuestions(): Promise<QuestionCard[]> {
  try {
    ...
    const data: SheetyResponse = await response.json();
    return shuffleArray(data.sheet1);
  } catch (error) {
    console.error('Error fetching questions:', error);
    throw error;
  }
